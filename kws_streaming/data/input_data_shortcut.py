...
class AudioHandler(object):
    ...
    for set_index in dirs:
      ...
      for mmap_path in search_path:
        ...
        if word in wanted_words_index:
          ...
          loaded_feature_number = len(self.loaded_features) - 1
          
          # TODO: Optimize this
          for i in range(0,len(imported_features)):
            self.data_index[set_index].append({'label': word, 'feature_number': loaded_feature_number, 'subindex': i})
      
      
      # # Look through all the subfolders in set_index to find audio samples
      # search_path_directory = os.path.join(self.data_dir, set_index)
      # search_path = [str(i) for i in Path(os.path.abspath(search_path_directory)).glob("**/*.npy")]
      ...
      #   if word in wanted_words_index:
      #     # this works for npy files
      #     numpy_array = np.load(wav_path, mmap_mode="r")
      #     self.loaded_features.append(numpy_array.copy())
          
      #     loaded_feature_number = len(self.loaded_features) - 1
          
      #     if (numpy_array.ndim == 2):
      #       self.data_index[set_index].append({'label': word, 'feature_number': loaded_feature_number, 'subindex': None})
      #       # self.data_index[set_index].append({'label': word, 'file': wav_path, 'subindex': None})
      #     elif (numpy_array.ndim == 3):
      #       for i in range(0, numpy_array.shape[0]):
      #         self.data_index[set_index].append({'label': word, 'feature_number': loaded_feature_number, 'subindex': i})
      #         # self.data_index[set_index].append({'label': word, 'file': wav_path, 'subindex': i})
        else:
          raise Exception('Unknown word ' + word)

      if not all_words:
        raise IOError('No .npy found at ' + search_path)
      for index, wanted_word in enumerate(wanted_words):
  ...
  def get_data(self, how_many, offset, flags, background_frequency,
    ...
    desired_spectrogram_length = flags.spectrogram_length
    ...
    for i in xrange(offset, offset + sample_count):
      ...
      numpy_features = self.loaded_features[sample['feature_number']][sample['subindex']]
      
      data_length = numpy_features.shape[0]
      features = flags.mel_num_bins
      
      # If the loaded spectrogram is longer than desired, we randomly choose a subset
      if (data_length > desired_spectrogram_length):
        features_offset = np.random.randint(0, data_length - desired_spectrogram_length)
      else:
        features_offset = 0   
      clipped_data = numpy_features[features_offset:(features_offset+desired_spectrogram_length)]
      data[i - offset, :] = clipped_data[:,0:features]
      label_index = self.word_to_index[sample['label']]
      labels[i - offset] = label_index

      
      # if sample['subindex'] != None:
      #   numpy_features = numpy_features[sample['subindex']]
...
class AudioProcessor(object):
  ...
  def prepare_split_data_index(self, wanted_words, split_data, file_ext):
    ...
    for set_index in dirs:
      ...
      search_path = os.path.join(
          os.path.join(self.data_dir, set_index), '*.pkl')
      for pkl_path in gfile.Glob(search_path):
        _, word = os.path.split(os.path.dirname(wav_path))
        word = word.lower()
        # Treat the '_background_noise_' folder as a special case,
        # it contains long audio samples we mix in to improve training.
        ...
        # If it's a known class, store its detail, otherwise raise error
        if word in wanted_words_index:
          self.data_index[set_index].append({'label': word, 'features': pkl_path})
        else:
          raise Exception('Unknown word ' + word)
  ...
  def prepare_processing_graph(self, flags):
    ...
    with tf.get_default_graph().name_scope('data'):
      ...
      if flags.wav:
        ...
        # wav_loader = io_ops.read_file(self.wav_filename_placeholder_)   
        # print(wav_loader)     
        # wav_data = np.load(wav_loader)
        # print(wav_data)
        
      if flags.preprocess == 'none':
        # numpy_tensor = wav_data.eval()
        # if (numpy_tensor.shape[0] > desired_samples):
        #   rn = np.random.randint(0, numpy_tensor.shape[0] - desired_samples)
        #   self.output_ = wav_data[rn:rn+desired_samples]
        # else:
        # data_length = wav_data.shape[1]
        # features_offset = np.random.randint(
        #     0,
        #     data_length - flags.desired_samples)
        # background_clipped = background_samples[background_offset:(
        #     background_offset + desired_samples)]        
        self.output_ = wav_data
